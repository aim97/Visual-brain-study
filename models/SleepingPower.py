...
class Model(nn.Module):
  def __init__(
    ...
    super().__init__()
    self.powerImage = LogPowerSpectrum() if spec_type == 'stft' else LogWaveletCWT((812.5 / np.linspace(5, 95, 100)).tolist())
    ...
    self.resize = None if resize is None else nn.Upsample(size=(resize, resize), mode='bilinear')
