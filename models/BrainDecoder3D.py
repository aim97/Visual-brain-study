...
class R2Plus1D_Block(nn.Module):
    def __init__(
        self, in_ch, out_ch, k_t=3, k_s=3, stride_t=1, stride_s=1, p_t=1, p_s=1
    ):
        super().__init__()
...
class EEG3DNet(nn.Module):
    def __init__(self, n_classes=40, base=32):
        ...
        self.layer2 = R2Plus1D_Block(
            base, base * 2, stride_t=2, stride_s=2
        )  # T: /2, H,W: 16->8
        self.layer3 = R2Plus1D_Block(
            base * 2, base * 2, stride_t=1, stride_s=2
        )  # H,W: 8->4
        self.layer4 = R2Plus1D_Block(
            base * 2, base * 4, stride_t=2, stride_s=2
        )  # T: /2, H,W: 4->2
