...
class ViT(nn.Module):
    def __init__(self,
                 ...
                 dim_head=None,
                 dropout=0, transformer=None, classification=True, classification_structure=True):
        """
        Args:
...
class ResizeTo64(nn.Module):
    ...
    def __init__(self, mode: str = "bilinear", align_corners: bool = False):
        ...
        if mode not in ("bilinear", "bicubic", "nearest"):
            raise ValueError(f"Unsupported mode '{mode}'. Choose from 'bilinear', 'bicubic', 'nearest'.")
        self.mode = mode
...
class ResNet18Wrapper(nn.Module):
    def __init__(self, num_classes: int):
        ...
        self.resnet.conv1 = nn.Conv2d(128, 64, kernel_size=7, stride=2, padding=3, bias=False)
